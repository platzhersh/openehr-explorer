...
// pattern instead of reimplementing it. Shows a small tooltip on hover
// ("Copy to clipboard" or the custom `title`); after a click it swaps to a
// "Copied!" tooltip and a checkmark icon, both reverting a couple seconds
// later.
import { ref } from "vue";
...
<template>
  <span class="copy-icon-wrap">
    ...
    <span class="copy-tooltip" :class="{ show: copied }" aria-hidden="true">
      {{ copied ? "Copied!" : title }}
    </span>
  </span>
</template>
...
.copy-icon-wrap:hover .copy-tooltip,
.copy-icon-wrap:focus-within .copy-tooltip,
.copy-tooltip.show {
  opacity: 1;
  visibility: visible;
