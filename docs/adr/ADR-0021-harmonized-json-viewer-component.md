...
- **Non-JSON raw text (`RequestInspector.vue` only):** `RequestInspector.vue` stores request/response bodies as raw strings and tries to `JSON.parse` them; a body that fails to parse (XML, plain text, an error message) stays `null`. `JsonViewer` only ever receives a successfully parsed value — call sites keep their existing plain `<pre>` fallback for the `null` case, so a non-JSON body is never coerced into (and re-quoted/escaped as) a JSON string value.
...
- `CompositionViewer.vue` (JSON tab, and the FLAT tab — this is the FLAT-*format* composition, a denormalised but still-JSON key/value object, rendered the same way as the JSON tab; not to be confused with the FLAT-*paths* list below)
...
Out of scope — none of these render raw JSON text: `highlightXml()` (XML highlighting in `TemplateBrowser.vue`/`RequestInspector.vue`); `TemplateBrowser.vue`'s "FLAT Paths" tab and `FlatPathPanel.vue` (both a plain list of path *strings*, not a JSON value); `RequestInspector.vue`'s FLAT view (a path/value table).
